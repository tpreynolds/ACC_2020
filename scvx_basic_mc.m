...
C.f_iq    = @(x)(C.b_iq - dot(C.d_iq,x));   % f_iq(x) <= 0;
C.c_eq    = [1;2;-1.2;-2;0];
...
save(strcat('data/mc_scvx_tr_',S.type))
...
plot(X1,C.f_eq(X1),'k','LineWidth',1,'HandleVisibility','off')
if (S.ineq)
    area([C.a1(1) C.a2(1)],[C.a1(2) C.a2(2)],-3,'FaceColor','k','FaceAlpha',0.3,'HandleVisibility','off')
end
...
for i = 1:MC.N_x
    for j = 1:MC.N_y
        ...
        elseif (MC.result{i,j}==id_infeas)
            plot(MC.ic{i,j}(1),MC.ic{i,j}(2),'kx',...
                'MarkerSize',6,'HandleVisibility','off')
        else
            plot(MC.ic{i,j}(1),MC.ic{i,j}(2),'ko',...
                'MarkerSize',5,'HandleVisibility','off')
        end
    end
end
contour(X1,X2,C.f_cost(X,Y),-4:0.2:4,'k:','HandleVisibility','off')
set(gca,'Xlim',[-2,2],'Ylim',[-2,2]);
xlabel('$x_1$','FontSize',16)
ylabel('$x_2$','FontSize',16)
title('SCvx Method: $2$-norm','FontSize',16)
