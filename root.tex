...
This paper is organized as follows. First, we describe the generic class of SCP algorithms studied in~\sref{sec:scp}, simultaneously defining the nomenclature and notation that is used throughout. In~\sref{sec:crawling} we introduce the crawling phenomenon and provide a simple example of how it can impact the solution of a non-convex optimization problem. Section~\ref{sec:remedies} then provides some discussion on algorithms that may avoid the crawling phenomenon, and~\sref{sec:conclusion} offers concluding remarks.
