...
\node[anchor=south,name=ncvx,blue] at (-1,0.5) {$h_1(\z)=0$};
...
\node[anchor=north,name=cvx,red] at (-0.5,-0.25) {$\bar{h}_1(\z,\zb)=0$};
% draw inequality g1
...
\node[anchor=south,darkgreen] at (-1,1.1) {$g_1(\z) \leq 0$};
% draw inequality g2
\draw[thick,darkgreen] (\bx,-1) -- (\bx,1.5);
\draw[darkgreen,->] (\bx-0.1,-0.8) -- ++(-0.4,0); 
\node[anchor=north,darkgreen] at (\bx,-1) {$g_2(\z)\leq0$};
